class StadisticsController < BaseController
  ...
  #Graphics maximum allowed
  $max = 9
  #Array containing the graphics
  $graphics = [];
  $stat = Stats.new([0,1,2,3,4,5])
  ...
  def destroy
    id_del = params[:id_del].to_i
    $graphics.delete_at(id_del)
  ...
  def create_product_chart(params)
    ...
    graphic = ProductGraphic.new(initial_date,final_date,chart_type) #Creates the graphic with the data sent.
    graphic.generate
    return graphic
  end
  ...
  def create_producer_chart(params)
    ...
    graphic = ProducerGraphic.new(chart_type) #Sends the params to create a new Producer Graphic.
    if option == "1"
  ...
  def create_hub_chart(params)
    ...
    graphic = HubGraphic.new(chart_type) #Sends the data to the constructor and creates a new hubs graphic.
    if option == "1"
  ...
  def create_user_chart(params)
    ...
    graphic = UserGraphic.new(chart_type) #Sends the data to the constructor and creates a new hubs graphic.
    if option == "1"
  ...
  def create_general_chart(params)
    ...
    graphic  = GeneralGraphic.new(chart_type)
    if option == "1"
  ...
  #This method replace the old stat for the new one selected
  #@param del_stat actual indexes
  def generate_stat
